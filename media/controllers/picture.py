...
class PictureController(BasicCRUD):
    def __init__(self):
        super().__init__(resource="picture", model=models.Picture,
    ...
    def create(body):
        ...
        if not body.get("storage_id"):
            body["storage_id"] = AccountSettings(
                account_id, config, models, g.db).get.default_storage_id
        ...
        if retval[1] == 201 and album_id:
            ...
            count = _get_count(g.db.query(models.AlbumContent).filter_by(
                album_id=album_id))
            g.db.add(models.AlbumContent(
                album_id=album_id, picture_id=picture_id,
                rank=rank or count + 1))
            if count == 0:
                album = g.db.query(models.Album).filter_by(id=album_id).one()
    ...
    def update(id, body):
        ...
        if retval[1] == 201 and album_id and rank:
            record = g.db.query(models.AlbumContent).filter(
                models.AlbumContent.album_id == album_id,
                models.AlbumContent.picture_id == id).one()
            record.rank = rank
            g.db.commit()
        return retval
